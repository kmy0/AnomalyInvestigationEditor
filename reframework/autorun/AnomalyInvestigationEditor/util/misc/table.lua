...
---@generic K, V
---@param t {[K]: V}
---@param keys K[] | fun(key: K, value: V): boolean
---@return {[K]: V}
function this.extract_keys(t, keys)
    local ret = {}
...
---@param keys K[] | fun(key: K, value: V): boolean
---@return V[]
function this.extract_values(t, keys)
    local ret = {}
...
---@param int integer
---@param sorted_ints integer[]
---@return integer?
function this.binary_search(int, sorted_ints)
    local left, right = 1, #sorted_ints
    local ret

    while left <= right do
        local mid = math.floor((left + right) / 2)
        if sorted_ints[mid] <= int then
            ret = sorted_ints[mid]
            left = mid + 1
        else
            right = mid - 1
...
---@return T[]
function this.fill(t, data, max_size)
    local size = 0
    while size ~= max_size do
        for j = 1, #data do
            table.insert(t, data[j])
            size = size + 1
